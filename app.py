...
st.markdown("<div id='main_content'>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.markdown("<div class='dashboard-container'><h2 style='text-align: center; color: black;font-size: 24px;'>Dashboard</h2></div>", unsafe_allow_html=True)
    ...
    # Format and display inference results
    inf_speed = st.session_state.inf_speed
    summary = st.session_state.summary
    summary = summary[len(summary)-1] if len(summary) > 0 else ''
    summary = summary[:-3] if summary.endswith('ss, ') else summary
    summary = summary[:-2] if summary.endswith(', ') else summary
